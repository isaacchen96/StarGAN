...
class StarGAN:
    ...
    def gen_train_step(self, natural_source, expression_source, train=True, direction='N2E'):
        ...
        with tf.GradientTape() as tape:
            ...
            gen_img2 = self.generator.call([gen_img, source_label])
            v_gen, c_gen = self.discriminator.call(gen_img)
            loss_img = reconstruction_loss(source, gen_img2)
    ...
    def train(self, epochs=20, interval=1, batch_size=32, batch_num=105):
        tr_L_G_avg = []
        te_L_G_img_avg = []
        te_L_G_adv_avg = []
        te_L_G_cls_avg = []
        ...
        for epoch in range(epochs):
            ...
            for b in range(batch_num):
                ...
                for direction in ['N2E', 'E2N']:
                    ...
                    loss_d_test, loss_adv_d, loss_cls_d = self.dis_train_step(natural_test, expression_test, train=False)
                    for i in range(2):
            ...
            if (epoch % interval == 0 or epoch + 1 == epochs) and (te_L_G_avg[-1] <= np.mean(te_L_G_avg)):
                self.generator.save_weights('weight/Xnew_gen_weights_{}'.format(epoch + 1))
                self.discriminator.save_weights('weight/Xnew_dis_weights_{}'.format(epoch + 1))

        return [tr_L_G_avg, te_L_G_adv_avg, te_L_G_img_avg, te_L_G_cls_avg], \
               [tr_L_D_avg, te_L_D_adv_avg, te_L_D_cls_avg], [te_L_G_avg, te_L_D_avg]

    def sample_image(self, epoch, path='picture/Xnew_'):
...
if __name__ == '__main__':
    stargan = StarGAN()
    stargan.generator.load_weights('pretrain_weight/new_2ck_gen_weights_10')
    stargan.discriminator.load_weights('pretrain_weight/new_2ck_dis_weights_10')
    [tr_L_G_avg, te_L_G_adv_avg, te_L_G_img_avg, te_L_G_cls_avg], [tr_L_D_avg, te_L_D_adv_avg, te_L_D_cls_avg], \
    [te_L_G_avg, te_L_D_avg] = stargan.train(epochs=50, interval=1)

    plt.plot(tr_L_G_avg)
    plt.plot(te_L_G_avg)
    plt.legend(['Train', 'Test'])
    plt.title('Generator total loss')
    plt.savefig('picture/Xnew_Generator loss.jpg')
    plt.close()
    ...
    plt.savefig('picture/Xnew_Adversarial loss.jpg')
    plt.close()

    plt.plot(te_L_G_img_avg)
    plt.title('Generator Image loss')
    plt.savefig('picture/Xnew_Generator Image loss.jpg')
    plt.close()

    plt.plot(te_L_G_cls_avg)
    plt.title('Generator Classify loss')
    plt.savefig('picture/Xnew_Generator Classify loss.jpg')
    plt.close()
    ...
    plt.savefig('picture/Xnew_Discriminator loss.jpg')
    plt.close()

    plt.plot(te_L_D_cls_avg)
    plt.title('Discriminator Classify loss')
    plt.savefig('picture/Xnew_Discriminator Classify loss.jpg')
    plt.close()
